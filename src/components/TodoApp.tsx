...
1. worktree "${ticket.worktreePath}" でbase_commit "${ticket.baseCommit}" からの差分をpatchファイルとして生成:
   cd "${ticket.worktreePath}" && git diff ${ticket.baseCommit} > /tmp/plx/plx-patch-${ticketId}.patch
2. メインディレクトリ "${dir.path}" のリポジトリルートを特定し、そこからpatchを適用:
   REPO_ROOT=$(cd "${dir.path}" && git rev-parse --show-toplevel) && cd "$REPO_ROOT" && git apply --3way /tmp/plx/plx-patch-${ticketId}.patch
...
1. patchファイル /tmp/plx/plx-patch-${ticketId}.patch が残っていればそれを使ってリポジトリルートから元に戻す:
   REPO_ROOT=$(cd "${dir.path}" && git rev-parse --show-toplevel) && cd "$REPO_ROOT" && git apply --reverse /tmp/plx/plx-patch-${ticketId}.patch
